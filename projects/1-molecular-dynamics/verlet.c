...
int VerletCreate(Verlet *verlet)
{
  int err;
  Verlet v;

  err = safeMALLOC(sizeof(*v), &v);
  CHK(err);
  v->d = 0.;
  v->rand = NULL;
  v->accel = NULL;
...
int VerletSetNoise(Verlet v, cse6230rand_t *rand, double d)
{
  v->d = d;
  v->rand = rand;
  return 0;
...
int VerletSetAccel(Verlet v, Accel accel)
{
  v->accel = accel;
  return 0;
}

int VerletDestroy(Verlet *verlet)
{
  free(*verlet);
  *verlet = 0;
  return 0;
