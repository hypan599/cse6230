...
int process_options(int argc, char **argv, int *Np, int *Nt, int *Nint, double *dt, double *k, double *d, double *L, double *r, const char **gifname)
{
    if (argc < 8 || argc > 10)
    {
...
int main(int argc, char **argv)
{
    ...
    err = VectorCreate(Np, &X0);
    CHK(err);
    err = VectorCreate(Np, &X);
    CHK(err);
    err = VectorCreate(Np, &U);
    CHK(err);
    ...
    err = VerletCreate(&Vr);
    CHK(err);
    err = VerletSetNoise(Vr, &rand, d);
    CHK(err);

    int use_ix = 1;
    err = AccelCreate(Np, L, k, r, use_ix, &Ac);
    CHK(err);
    ...
    if (gifname)
    {
        write_step(X, L, Nt * dt);
    }

    err = AccelDestroy(&Ac);
    CHK(err);
    err = VerletDestroy(&Vr);
    CHK(err);
    err = VectorDestroy(&U);
    CHK(err);
    err = VectorDestroy(&X);
    CHK(err);
    err = VectorDestroy(&X0);
    CHK(err);
    return 0;
}
