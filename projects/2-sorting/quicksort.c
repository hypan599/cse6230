...
static int ChoosePivot(Proj2Sorter sorter, MPI_Comm comm, size_t numKeysLocal, uint64_t *keys, uint64_t *pivot_p)
{
  ...
  err = MPI_Bcast(&pivot, 1, MPI_UINT64_T, 0, comm);
  MPI_CHK(err);
  *pivot_p = pivot;
  return 0;
}
...
static int Proj2SorterSort_quicksort_recursive(Proj2Sorter sorter, MPI_Comm comm,
                                               size_t numKeysLocal, size_t *numKeysLocalReturn,
                                               uint64_t *keys, uint64_t *keysReturn)
{
  uint64_t pivot = 0;
  ...
  {
    if (pivot < keys[0])
    { /* if the pivot is less than the first entry, they all are in the upper half */
      upper_half = keys;
      upper_size = numKeysLocal;
    }
    else if (pivot >= keys[numKeysLocal -
                           1])
    { /* if the pivot is greater than or equal to the last entry, they all are in the lower half */
      lower_half = keys;
      lower_size = numKeysLocal;
    }
    else
    { /* otherwise use a binary search to split the keys */
      lower_half = keys;
      upper_half = (uint64_t *)bsearch(&pivot, keys, numKeysLocal - 1, sizeof(uint64_t), uint64_compare_pair);
      if (!upper_half)
        PROJ2ERR(MPI_COMM_SELF, 5, "Could not find pivot\n");
  ...
  if ((size % 2) == 0 || (rank != size - 1))
  {
    ...
    if (equivRank > rank)
    {
      ...
      err = MPI_Isend(upper_half, (int)upper_size, MPI_UINT64_T, equivRank, PROJ2TAG_QUICKSORT, comm, &sendreq);
      MPI_CHK(err);
      err = MPI_Probe(equivRank, PROJ2TAG_QUICKSORT, comm, &recvstatus);
      MPI_CHK(err);
      ...
      err = MPI_Recv(&keysNew[lower_size], numIncoming, MPI_UINT64_T, equivRank, PROJ2TAG_QUICKSORT, comm,
                     MPI_STATUS_IGNORE);
      PROJ2CHK(err);
    ...
    else
    {
      ...
      err = MPI_Isend(lower_half, (int)lower_size, MPI_UINT64_T, equivRank, PROJ2TAG_QUICKSORT, comm, &sendreq);
      MPI_CHK(err);
      err = MPI_Probe(equivRank, PROJ2TAG_QUICKSORT, comm, &recvstatus);
      MPI_CHK(err);
      err = MPI_Get_count(&recvstatus, MPI_UINT64_T, &numIncoming);
  ...
  if (size % 2)
  { /* odd number of processes: size - 1 has been left out */
    MPI_Request sendreq;
    MPI_Status recvstatus;
    ...
    if (rank == (size / 2) - 1)
    {
      ...
      /* concatenate the addition from (size - 1) to the already constructed
             * keysNew */
      err = MPI_Probe(size - 1, PROJ2TAG_QUICKSORT, comm, &recvstatus);
      MPI_CHK(err);
      err = MPI_Get_count(&recvstatus, MPI_UINT64_T, &numIncoming);
      MPI_CHK(err);
      ...
      PROJ2CHK(err);
      err = MPI_Recv(&keysNewNew[numKeysLocalNew], numIncoming, MPI_UINT64_T, size - 1, PROJ2TAG_QUICKSORT, comm,
                     MPI_STATUS_IGNORE);
      PROJ2CHK(err);
    ...
    else if (rank == (size - 1))
    {
      ...
      err = MPI_Isend(lower_half, (int)lower_size, MPI_UINT64_T, (size / 2) - 1, PROJ2TAG_QUICKSORT, comm,
                      &sendreq);
      MPI_CHK(err);
  ...
  PROJ2CHK(err);
  err = Proj2SorterSort_quicksort_recursive(sorter, subcomm, numKeysLocalNew, keysNew);
  PROJ2CHK(err);
  ...
  err = Proj2SorterRestoreWorkArray(sorter, numKeysLocalNew, sizeof(uint64_t), &keysNew);
  PROJ2CHK(err);
  *numKeysLocalReturn = numKeysLocalNew;
...
int Proj2SorterSort_quicksort_redistribute(Proj2Sorter sorter, MPI_Comm comm,
                                           size_t numKeysLocalOld, size_t numKeysLocalNew,
                                           uint64_t *keys, uint64_t *keysNew)
{
  ...
  err = Proj2SorterGetWorkArray(sorter, size + 1, sizeof(uint64_t), &oldOffsets);
  PROJ2CHK(err);
  err = Proj2SorterGetWorkArray(sorter, size + 1, sizeof(uint64_t), &newOffsets);
  PROJ2CHK(err);
  err = MPI_Allgather(&myOldCount, 1, MPI_UINT64_T, oldOffsets, 1, MPI_UINT64_T, comm);
  PROJ2CHK(err);
  err = MPI_Allgather(&myNewCount, 1, MPI_UINT64_T, newOffsets, 1, MPI_UINT64_T, comm);
  PROJ2CHK(err);
  ...
  err = Proj2SorterGetWorkArray(sorter, lastRecv - firstRecv, sizeof(*recv_reqs), &recv_reqs);
  PROJ2CHK(err);
  ...
  for (int i = firstRecv; i < lastRecv; i++)
  {
    ...
    {
      err = MPI_Irecv(&keys[thisOffset - oldOffsets[rank]], (int)(recvEnd - recvStart), MPI_UINT64_T, i,
                      PROJ2TAG_QUICKSORT, comm, &recv_reqs[nRecvs++]);
      MPI_CHK(err);
    }
    thisOffset = recvEnd;
  }
  ...
  err = Proj2SorterGetWorkArray(sorter, lastSend - firstSend, sizeof(*send_reqs), &send_reqs);
  PROJ2CHK(err);
  ...
  for (int i = firstSend; i < lastSend; i++)
  {
    ...
    {
      err = MPI_Isend(&keysNew[thisOffset - newOffsets[rank]], (int)(sendEnd - sendStart), MPI_UINT64_T, i,
                      PROJ2TAG_QUICKSORT, comm, &send_reqs[nSends++]);
      MPI_CHK(err);
    }
    thisOffset = sendEnd;
  }

  err = MPI_Waitall(nRecvs, recv_reqs, MPI_STATUSES_IGNORE);
  PROJ2CHK(err);
  err = MPI_Waitall(nSends, send_reqs, MPI_STATUSES_IGNORE);
  PROJ2CHK(err);

  err = Proj2SorterRestoreWorkArray(sorter, lastSend - firstSend, sizeof(*send_reqs), &send_reqs);
  PROJ2CHK(err);
  err = Proj2SorterRestoreWorkArray(sorter, lastRecv - firstRecv, sizeof(*recv_reqs), &recv_reqs);
  PROJ2CHK(err);
  err = Proj2SorterRestoreWorkArray(sorter, size + 1, sizeof(uint64_t), &newOffsets);
  PROJ2CHK(err);
...
int Proj2SorterSort_quicksort(Proj2Sorter sorter, size_t numKeysLocal, int uniform, uint64_t *keys)
{
  ...
  int *numKeysLocalNew;
  unit64_t *keysNew = NULL;
  err = Proj2SorterSort_quicksort_recursive(sorter, sorter->comm, numKeysLocal, keys);
  PROJ2CHK(err);
  err = Proj2SorterSort_quicksort_redistribute(sorter, sorter->comm, numKeysLocal, keys);
  PROJ2CHK(err);
