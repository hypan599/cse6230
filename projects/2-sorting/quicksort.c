...
static int Proj2SorterSort_quicksort_recursive(Proj2Sorter sorter, MPI_Comm comm, size_t numKeysLocal, uint64_t *keys)
{
  ...
  err = Proj2SorterSortLocal(sorter, numKeysLocal, keys, PROJ2SORT_FORWARD, 1);
  PROJ2CHK(err);
  if (size == 1)
  {
  ...
  if (numKeysLocal)
  {
    ...
    else if (pivot >= keys[numKeysLocal - 1])
   ...
   * distribution */
  {
    ...
    for (int i = firstRecv; i < lastRecv; i++)
    {
      ...
      {
        err = MPI_Irecv(&keys[thisOffset - oldOffsets[rank]], (int)(recvEnd - recvStart), MPI_UINT64_T, i, PROJ2TAG_QUICKSORT, comm, &recv_reqs[nRecvs++]);
        MPI_CHK(err);
      }
      thisOffset = recvEnd;
    }
    ...
    for (int i = firstSend; i < lastSend; i++)
    {
      ...
      {
        err = MPI_Isend(&keysNew[thisOffset - newOffsets[rank]], (int)(sendEnd - sendStart), MPI_UINT64_T, i, PROJ2TAG_QUICKSORT, comm, &send_reqs[nSends++]);
        MPI_CHK(err);
      }
      thisOffset = sendEnd;
    }
