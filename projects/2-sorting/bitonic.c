...
static int uint64_swap_bitonic(Proj2Sorter sorter, size_t nkeys, uint64_t *keys, int start, int end, int rank, int direction)
{
  ...
  err = Proj2SorterGetWorkArray(sorter, nkeys, sizeof(uint64_t), &recv);
  PROJ2CHK(err);
  err = MPI_Irecv(recv, nkeys, MPI_UINT64_T, comm_rank, PROJ2TAG_BITONIC, sorter->comm, &recv_req);
  MPI_CHK(err);
  err = MPI_Isend(keys, nkeys, MPI_UINT64_T, comm_rank, PROJ2TAG_BITONIC, sorter->comm);
  MPI_CHK(err);
  ...
  err = Proj2SorterRestoreWorkArray(sorter, nkeys, sizeof(uint64_t), &recv);
  PROJ2CHK(err);
  return 0;
}
...
static int uint64_sort_bitonic(Proj2Sorter sorter, size_t nkeys, uint64_t *keys, int start, int end, int rank, int direction, int input_bitonic)
{
...
#if defined(LOG_BITONIC)
  if (diff > 1 && rank == start)
    PROJ2LOG(MPI_COMM_SELF, "[%d, %d),%zu,%d,%d\n", start, end, nkeys, direction, input_bitonic);
#endif
  ...
  if (diff == 1)
  {
    ...
    else if (nkeys > 1)
    {
      ...
      err = uint64_sort_bitonic(sorter, limit, keys, start, end, rank, direction, 1);
      PROJ2CHK(err);
      err = uint64_sort_bitonic(sorter, shift, &keys[limit], start, end, rank, direction, 1);
      PROJ2CHK(err);
    }
    return 0;
  }
...
#if defined(LOG_BITONIC)
    if (next_end - next_start == 1 && rank == next_start)
      PROJ2LOG(MPI_COMM_SELF, "[%d, %d),%zu,%d,0\n", next_start, next_end, nkeys, next_direction);
#endif
    err = uint64_sort_bitonic(sorter, nkeys, keys, next_start, next_end, rank, next_direction, input_bitonic);
    PROJ2CHK(err);
#if defined(LOG_BITONIC)
    if (diff > 1 && rank == start)
      PROJ2LOG(MPI_COMM_SELF, "[%d, %d),%zu,%d,1\n", start, end, nkeys, direction);
#endif
  }
  err = uint64_swap_bitonic(sorter, nkeys, keys, start, end, rank, direction);
  PROJ2CHK(err);
  {
    int pow2 = 1;
    int split;
...
#endif
    err = uint64_sort_bitonic(sorter, nkeys, keys, next_start, next_end, rank, direction, 1);
    PROJ2CHK(err);
  }
...
int Proj2SorterSortBitonic(Proj2Sorter sorter, size_t nkeys, int uniform, uint64_t *keys)
{
  ...
  if (!uniform)
  {
    PROJ2ERR(comm, 1, "Cannot use bitonic sort on non-uniform distributions\n");
  }
  err = uint64_sort_bitonic(sorter, nkeys, keys, 0, size, rank, PROJ2SORT_FORWARD, 0);
  PROJ2CHK(err);
