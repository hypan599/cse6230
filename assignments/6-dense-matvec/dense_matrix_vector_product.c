...
static int GetNorm(MPI_Comm comm, int nLocal, double *entries, double *norm)
{
  ...
  {
    localNorm2 += entries[i] * entries[i];
  }

  err = MPI_Allreduce(&localNorm2, &globalNorm2, 1, MPI_DOUBLE, MPI_SUM, comm);
  MPI_CHK(err);
...
int main(int argc, char **argv)
{
  ...
  MPI_CHK(err);
  err = ArgsCreate(MPI_COMM_WORLD, argc, argv, &args);
  MPI_CHK(err);
  ...
  err = VectorsGetLocalSize(args, &lLocal, &rLocal);
  MPI_CHK(err);
  ...
  err = VecGetOffset(args, rLocal, &rOffset);
  MPI_CHK(err);

  /* Now that we know the indices, fill the values of the local portion of the
   * right vector */
  err = VecGetEntries(args, rOffset, rOffset + rLocal, vecRightLocal);
  MPI_CHK(err);
  rStart = rOffset;
  rEnd = rOffset + rLocal;
  err = VecGetOffset(args, lLocal, &lOffset);
  MPI_CHK(err);
  lStart = lOffset;
  lEnd = lOffset + lLocal;
   ...
   * should be independent of the partition */
  {
    ...
    err = GetNorm(MPI_COMM_WORLD, rLocal, vecRightLocal, &rightNorm);
    MPI_CHK(err);
    if (!rank)
    {
  ...
  err = ArgsGetPartitionType(args, &partitionType);
  MPI_CHK(err);
  ...
  case PARTITION_ROWS:
  {
    ...
    err = VecGetGlobalSize(args, rLocal, &rGlobal);
    MPI_CHK(err);
  ...
  case PARTITION_COLS:
  {
    /* We need to know the global size of the right vector so that we can
       * allocate and fill complete rows of the matrix */
    err = VecGetGlobalSize(args, lLocal, &lGlobal);
    MPI_CHK(err);
  ...
  case PARTITION_2D:
  {
    ...
    err = VecGetLayout(args, rLocal, rLayout);
    MPI_CHK(err);
    err = VecGetLayout(args, lLocal, lLayout);
    MPI_CHK(err);

    /* Use the vector layouts to determine the compatible 2d matrix partition */
    err = MatrixGetLocalRange2d(args, lLayout, rLayout, &mStart, &mEnd, &nStart, &nEnd);
    MPI_CHK(err);
    free(lLayout);
    free(rLayout);
  }
  ...
  err = MatrixGetEntries(args, mStart, mEnd, nStart, nEnd, matrixEntries);
  MPI_CHK(err);

  {
    ...
    err = GetNorm(MPI_COMM_WORLD, (mEnd - mStart) * (nEnd - nStart), matrixEntries, &matNorm);
    MPI_CHK(err);
    if (!rank)
    {
  ...
  for (int i = 0; i < numTests + 1; i++)
  {
    ...
    timer = tic();
    err = DenseMatVec(args, mStart, mEnd, nStart, nEnd, matrixEntries, rStart, rEnd, vecRightLocal, lStart, lEnd, vecLeftLocal);
    MPI_CHK(err);
    thistime = toc(&timer);
    if (i)
    {
...

  {
    ...
    err = GetNorm(MPI_COMM_WORLD, lLocal, vecLeftLocal, &resultNorm);
    MPI_CHK(err);
    if (!rank)
    {
