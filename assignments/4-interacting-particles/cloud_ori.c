...
main (int argc, char **argv)
{
  ...
  initialize_variables_ori (Np, k, &rand, X0, X, U);

  Hin = compute_hamiltonian_ori (Np, k, (const double **)X, (const double **)U);
  if (!gifname) {
  ...
  for (int t = 0; t < Nt; t += Nint) {
    if (gifname) {
      Hout = compute_hamiltonian_ori (Np, k, (const double **)X, (const double **)U);
      write_step (Np, t * dt, Hout, (const double **)X);
    }
    /* execute the loop */
    verlet_step_ori (Np, Nint, dt, k, d, &rand, X, U);
  }
